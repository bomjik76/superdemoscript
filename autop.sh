...
# Added for autop.sh automation
OSPF_PASSWORD_DEFAULT="YourFRRPassword123!" # For FRR configuration
MARIADB_ROOT_PASSWORD_DEFAULT="StrongPassword123!" # For MariaDB secure installation
...
configure_nftables() {
    ...
    # case "$choice" in 
    #     y|Y ) 
    # Default action: Install and configure nftables
...
configure_frr() {
    ...
    local ospf_password="$OSPF_PASSWORD_DEFAULT"
...
install_lms_apache() {
    ...
    echo "Securing MariaDB installation (setting root password to $MARIADB_ROOT_PASSWORD_DEFAULT)..."
    mysql -u root -e "UPDATE mysql.user SET Password = PASSWORD('$MARIADB_ROOT_PASSWORD_DEFAULT') WHERE User = 'root' AND (Password = '' OR Password IS NULL);"
    # Assuming the above command succeeded, subsequent commands might need -p. 
    # However, for simplicity and common initial setups, we'll assume root can execute these without -p initially, or with the new password.
    # This part is tricky and depends on exact MariaDB setup. For a fresh install, this sequence is common.
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "DELETE FROM mysql.user WHERE User='';"
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost', '127.0.0.1', '::1');"
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "DROP DATABASE IF EXISTS test;"
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%';" # Note: escaping % for mysql
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "FLUSH PRIVILEGES;"
    echo "MariaDB basic security applied."

    # Создание базы данных и пользователя
    echo "Создание базы данных и пользователя для LMS..."
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "CREATE DATABASE $MOODLE_DB DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;"
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "CREATE USER '$MOODLE_USER'@'localhost' IDENTIFIED BY '$MOODLE_PASS';"
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "GRANT ALL ON $MOODLE_DB.* TO '$MOODLE_USER'@'localhost';"
    mysql -u root -p"$MARIADB_ROOT_PASSWORD_DEFAULT" -e "FLUSH PRIVILEGES;"
    # Установка Moodle
...
configure_ansible() {
    ...
    ssh-keygen -t rsa -N '' -f ~/.ssh/id_rsa <<< y # Non-interactive keygen
    # Копирование SSH-ключей на удаленные устройства
    # This part (ssh-copy-id) is inherently interactive if passwords are required.
    # For full automation, passwordless SSH or expect scripts would be needed setup beforehand.
    # We will assume ssh-keys can be copied or this step is handled manually for true automation.
    echo "Копирование SSH-ключей на удаленные устройства (может потребоваться ввод пароля)..."
    echo "ssh-copy-id -p $ANSIBLE_SSH_PORT $ANSIBLE_SSH_USER@$ANSIBLE_HQ_SRV_IP"  # HQ-SRV
    echo "ssh-copy-id $ANSIBLE_USER_CLI@$ANSIBLE_HQ_CLI_IP"  # HQ-CLI
    echo "ssh-copy-id $ANSIBLE_USER_RTR@$ANSIBLE_HQ_RTR_IP"  # HQ-RTR
    echo "ssh-copy-id $ANSIBLE_USER_RTR@$ANSIBLE_BR_RTR_IP"  # BR-RTR
...
EOF
    ...
    # join-to-domain.sh # This script might be interactive. For full automation, it needs to be non-interactive or its actions performed directly.
    echo "Attempting to join domain using join-to-domain.sh. This may require further automation if the script is interactive."
    # For now, we call it as is.
    if [ -f /usr/local/bin/join-to-domain.sh ]; then # Example path
        /usr/local/bin/join-to-domain.sh
    elif [ -f ./join-to-domain.sh ]; then
        ./join-to-domain.sh
    else
        echo "join-to-domain.sh not found. Skipping domain join."
    fi
    # read -p "Нажмите Enter для продолжения..."
}
